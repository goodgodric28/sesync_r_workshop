...
library(ggplot2)
surveys_rodent <- inner_join(surveys, species) %>%
  filter(taxa == 'Rodent') %>%
  group_by(species) %>%
  collect()

ggplot(data = surveys_rodent,
       aes(x = species)) +
...
```{r eval = TRUE, echo = FALSE}
plots <- tbl(db, 'plots')
surveys_1990 <- filter(surveys, year == 1990) %>%
  select(-year) %>%
  inner_join(species) %>%
  filter(taxa == 'Rodent') %>%
  inner_join(plots) %>%
...
ggplot(data = surveys_1990,
       aes(x = plot_type)) +
  geom_boxplot(aes(y = weight), color = 'lime green')
```
